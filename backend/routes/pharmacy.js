...
// @access  Private
router.post('/parse-prescription', protect, upload.single('image'), async (req, res) => {
  try {
    if (!req.file) {
...
router.get('/search/:drugName', protect, async (req, res) => {
  try {
    ...
    if (error.response?.status === 404) {
      return res.status(404).json({ message: `Drug "${drugName}" not found in FDA database. Try a different spelling or generic name.` });
    }
