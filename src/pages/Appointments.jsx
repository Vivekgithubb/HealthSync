...
import { Calendar, Plus, Trash2, Clock, Paperclip, Info, X, Bell } from 'lucide-react';
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-500 mt-2">Schedule and manage your appointments</p>
        </div>
        <button
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-2xl font-semibold text-blue-900">Schedule Appointment</h2>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div className="grid md:grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-blue-900 mb-2">Time *</label>
                    <input
                      type="time"
                ...
                <div>
                  <label className="block text-sm font-medium text-blue-900 mb-2">Attach Documents (Reports to bring)</label>
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-md p-3 flex items-start space-x-2">
                  ...
                  <p className="text-sm text-blue-800">
                    You will receive an email reminder for appointments scheduled within the next 2 days.
                  </p>
                </div>
      ...
      {appointments.length === 0 ? (
        <div className="bg-white rounded-lg shadow-md p-12 text-center border border-gray-200">
          ...
          <h3 className="text-xl font-semibold text-blue-900 mb-2">No appointments scheduled</h3>
          <p className="text-gray-500 mb-6">Schedule your first appointment</p>
          <button onClick={() => setShowForm(true)} className="bg-amber-500 text-white px-6 py-2 rounded-md font-semibold hover:bg-amber-600">Schedule Appointment</button>
        </div>
      ) : (
        <div className="grid md:grid-cols-2 gap-4">
          {appointments.map(a => (
            <div key={a._id} className="bg-white rounded-lg shadow-md p-5 border border-gray-200">
              <div className="flex justify-between items-start mb-3">
                <div>
                  <p className="text-blue-900 font-semibold text-lg">{a.doctor?.name || 'Unknown'}</p>
              ...
              <div className="space-y-2 mb-3">
                <p className="text-sm text-gray-700 flex items-center space-x-2">
                  ...
                  <span>{new Date(a.appointmentDate).toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</span>
                </p>
                <p className="text-sm text-gray-700 flex items-center space-x-2">
                ...
                {a.documents?.length > 0 && (
                  <p className="text-sm text-gray-500 flex items-center space-x-1">
                    ...
                    <span>{a.documents.length} document(s)</span>
                  </p>
                )}
              </div>
              <button
